...
package com.adobe.scenegraph
{
    ...
    //  Class
    // ---------------------------------------------------------------------------
    public class Line extends SceneRenderable
    {
        // ======================================================================
        //  Constants
        // ----------------------------------------------------------------------
        protected static const VERTEX_STRIDE:uint                   = 11;
        protected static const INDICES:Vector.<uint>                = new <uint>[
            0, 1, 2,
            3, 2, 1
        ];

        protected static const VERTEX_ASSIGNMENTS:Vector.<VertexBufferAssignment> = Vector.<VertexBufferAssignment>
            (
                [
                    new VertexBufferAssignment( 0, Context3DVertexBufferFormat.FLOAT_3 ),
                    new VertexBufferAssignment( 3, Context3DVertexBufferFormat.FLOAT_3 ),
                    new VertexBufferAssignment( 6, Context3DVertexBufferFormat.FLOAT_1 ),
                    new VertexBufferAssignment( 7, Context3DVertexBufferFormat.FLOAT_4 )
                ]
            );
        ...
        //  Constructor
        // ----------------------------------------------------------------------
        public function Line( instance:Instance3D, vertices:Vector.<Number>, vertexStride:uint, indices:Vector.<uint> )
        {
            _vertexStride = vertexStride
            _nVertices = vertices.length / _vertexStride;
            _nTriangles = indices.length / 3;

            _vertexBuffer = instance.createVertexBuffer( _nVertices, _vertexStride );
            _vertexBuffer.uploadFromVector( vertices, 0, _nVertices );

            _indexBuffer = instance.createIndexBuffer( _nTriangles * 3 );
            _indexBuffer.uploadFromVector( indices, 0, _nTriangles * 3 );

            _program = instance.createProgram();
